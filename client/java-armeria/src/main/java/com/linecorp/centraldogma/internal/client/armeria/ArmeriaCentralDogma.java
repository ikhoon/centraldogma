...
public final class ArmeriaCentralDogma extends AbstractCentralDogma {
    ...
    private static void appendJsonPaths(StringBuilder path, QueryType queryType, Iterable<String> expressions) {
        if (queryType == com.linecorp.centraldogma.common.QueryType.JSON_PATH) {
        ...
        if (viewRaw) {
            ...
            if (rawContent != null) {
                try {
                    ...
                    // Should never reach here as it was already validated by the server.
                    throw new IllegalStateException(e);
                }
            }
            logger.warn("The server does not support raw content. path: {}", entryPath);
