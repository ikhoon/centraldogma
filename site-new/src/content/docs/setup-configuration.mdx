---
title: Configuration
description: Every property of conf/dogma.json, from ports and TLS to replication, plugins and management.
---
...
- `tls`
  ...
  - the configuration for Transport Layer Security(TLS) support. Specify `null` to disable TLS.
    See [Configuring TLS](#configuring-tls) for more information.

- `trustedProxyAddresses` (string array)

  - the addresses or ranges of [Classless Inter-domain Routing (CIDR)](https://tools.ietf.org/html/rfc4632)
...
- `clientAddressSources` (string array)
  ...
  - the HTTP header names to be used for retrieving a client address. `PROXY_PROTOCOL` is a reserved keyword
    ...
    [PROXY protocol](https://www.haproxy.org/download/1.8/doc/proxy-protocol.txt) message.
...
- `repositoryCacheSpec` (string)
  ...
  - the cache specification string which determines the capacity and behavior of the repository
    access cache. Refer to [the Caffeine API documentation](https://static.javadoc.io/com.github.ben-manes.caffeine/caffeine/2.6.2/com/github/benmanes/caffeine/cache/CaffeineSpec.html) for more information.
...
- `replication`
  ...
  - `method` (string)
    ...
    - the replication method. `NONE` indicates 'standalone mode' without replication. See [Configuring replication](#configuring-replication)
...
- `accessLogFormat` (string)
  ...
  - the format to be used for writing an access log. `common` and `combined` are pre-defined for NCSA
    ...
    here. Read [Writing an access log](https://line.github.io/armeria/docs/server-access-log) for more
...
- `authentication`
  ...
  - the authentication configuration. If `null`, the authentication is disabled.
    See [Authentication and access control](auth.mdx) to learn how to configure the authentication layer.
...
- `pluginConfigs`
  ...
  - the list of plugin configuration. See [Configuring plugins](#configuring-plugins) for more information.

- `management`

  - the management server configuration. Read [ManagementService API documentation](https://javadoc.io/doc/com.linecorp.armeria/armeria-javadoc/latest/com/linecorp/armeria/server/management/ManagementService.html)
...
- `zone`
    ...
    - `currentZone` (string)
      ...
      - You can also dynamically load a zone name by implementing
        [com.linecorp.centraldogma.server.ConfigValueConverter](type).
...
Central Dogma implements multi-master replication by embedding [Apache ZooKeeper](https://zookeeper.apache.org).
...
`CHANGE_ME` is intentionally invalid. Replace it with a unique secret before starting any replica.
...
- `servers`
  ...
  - a map whose key is the `serverId` of a replica in the cluster and whose value is a map which
    ...
    - `groupId` (integer)
      ...
      - the group ID which is used by ZooKeeper for
        [hierarchical quorums](https://zookeeper.apache.org/doc/r3.5.8/zookeeperHierarchicalQuorums.html)
  ...
  - It is highly recommended to have more than 3, preferably odd number of, replicas because the consensus
    ...
    See [here](http://bytecontinnum.com/2016/09/zookeeper-always-configured-odd-number-nodes/) or
    [here](https://www.quora.com/HBase-Why-we-run-zookeeper-with-odd-number-of-instance) if you are
...
- `secret` (string)
  ...
  - the secret string which is used for replicas to authenticate each other. The replicas in the same
    ...
    The secret must be at least 16 characters long. Generate a different random secret for each cluster with
    `openssl rand -hex 32`. You can secure this property by
    [hiding sensitive property values](#hiding-sensitive-property-values).
...
- `additionalProperties` (map of string key-value pairs)
  ...
  - ZooKeeper [configuration properties](https://zookeeper.apache.org/doc/current/zookeeperAdmin.html#sc_configuration).
...
- `tls`
  ...
  - `keyCertChain` (string)
    ...
    - the content of the certificate chain. If you want to use a file, specify `file:<path>`.
      You can secure this property by [hiding sensitive property values](#hiding-sensitive-property-values).
...
- `trustedHostKeys` (object)
   ...
   - a map of hostname to a list of trusted SSH host key fingerprints (SHA-256) for `git+ssh://` mirrors.
     ...
     `ssh-keyscan` does not authenticate the key it retrieves. Before trusting the fingerprint, verify it
     out of band against the fingerprint published by the Git host owner.
...
For more information about mirroring, refer to [Mirroring](mirroring.mdx).
...
with your own [com.linecorp.centraldogma.server.ConfigValueConverter](type) and register it via
