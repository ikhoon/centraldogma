...
const MirrorForm = ({ projectName, defaultValue, onSubmit, isWaitingResponse }: MirrorFormProps) => {
  ...
  const repoOptions: OptionType[] = (repos || []).map((repo) => ({
    value: repo.name,
    label: repo.name,
  }));
    ...
    <form onSubmit={handleSubmit((mirror) => onSubmit(mirror, reset, setError))}>
      <Center>
        <VStack width="80%" align="left">
          <Heading alignSelf="center">{defaultValue?.index >= 0 ? 'Edit mirror' : 'New mirror'}</Heading>
          ...
          <Stack direction="row" width="100%">
            <FormControl width="50%" isRequired isInvalid={errors.remoteScheme != null}>
              <FormLabel>
                <LabelledIcon icon={GoRepo} text={'Remote repository'} />
              </FormLabel>
              <Controller
                control={control}
                name="remoteScheme"
